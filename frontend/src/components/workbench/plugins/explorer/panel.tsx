...
import type { BackendConnectionProfile, DiagnosticMarker, EditorTab, TerminalPreferences, WorkspaceRoot } from "../../core/types";
import { cn } from "@/lib/utils";

const panelTabs = ["Terminal", "Problems", "Output", "Ports"];
      ...
      {activeTab === "Terminal" ? (
        <div className="min-h-0 flex-1 overflow-hidden">
          <TerminalView
            ...
            onChangePreferences={onChangeTerminalPreferences}
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 font-mono text-sm text-[#d4d4d4]">
          ...
          {activeTab === "Ports" && (
            <div className="space-y-3">
              <div className="rounded border border-[#2a2d2e] bg-[#1e1e1e] p-3">
                <div className="text-xs uppercase tracking-wide text-[#8b8b8b]">Selected Backend</div>
                <div className="mt-1 text-sm text-[#d4d4d4]">{activeBackendProfile?.name ?? "No backend selected"}</div>
                {activeBackendProfile && <div className="mt-1 break-all text-xs text-[#8b8b8b]">{activeBackendProfile.httpBaseUrl}</div>}
              </div>
              <div className="rounded border border-[#2a2d2e] bg-[#1e1e1e] p-3">
