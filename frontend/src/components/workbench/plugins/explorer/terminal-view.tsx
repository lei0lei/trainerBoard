...
import { fetchTerminalCapabilities, getTerminalWebSocketUrl, type TerminalCapabilitiesResponse } from "../../core/api";
import type { BackendConnectionProfile, TerminalPreferences } from "../../core/types";
...
export function TerminalView({
  ...
  const wsUrl = useMemo(() => getTerminalWebSocketUrl(backendProfile), [backendProfile]);
    ...
    async function bootstrap() {
      try {
        ...
        const [xtermModule, fitModule, terminalCapabilities] = await Promise.all([
          ...
          fetchTerminalCapabilities(backendProfile),
        ]);
  ...
  }, [backendProfile, onChangePreferences, preferences.sshHost, preferences.sshKeyPath, preferences.sshPort, preferences.sshShell, preferences.sshUsername, preferences.windowsShell]);
  ...
  function connectTerminal() {
    ...
    const mode = capabilities.default_mode;
    setError(null);
  ...
  const mode = capabilities?.default_mode ?? "ssh";
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-[#2a2d2e] bg-[#1f1f1f] px-3 py-1 text-xs text-[#8b8b8b]">
        <span>{capabilities ? `${capabilities.default_mode} ? ${status}` : "terminal unavailable"}</span>
