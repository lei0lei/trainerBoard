...
export function LitegraphPanel() {
  ...
  const activeBackendProfile = backendProfiles.find((item) => item.id === activeBackendProfileId) ?? backendProfiles[0] ?? null;

  async function refreshWorkflows() {
    try {
      const items = await listLitegraphWorkflows(activeBackendProfile);
  ...
  useEffect(() => {
    ...
    const wsUrl = getLitegraphQueueWebSocketUrl(activeBackendProfile);
    if (!wsUrl) return;
    ...
    socket.onmessage = async (event) => {
      ...
      if (payload.latest_completed) {
        const fullItem = await getLitegraphQueueItem(payload.latest_completed, activeBackendProfile);
  ...
  async function handleSaveBackend() {
    ...
    setLoading(true);
    try {
      const result = await saveLitegraphWorkflow(litegraphWorkflowName, litegraphGraph, activeBackendProfile);
  ...
  async function handleRunQueue() {
    ...
    setLoading(true);
    try {
      const item = await enqueueLitegraphWorkflow(litegraphGraph, litegraphWorkflowName, activeBackendProfile);
  ...
  async function handleCancelQueueItem(itemId: string) {
    try {
      const item = await cancelLitegraphQueueItem(itemId, activeBackendProfile);
  ...
  async function handleRetryQueueItem(itemId: string) {
    try {
      const item = await retryLitegraphQueueItem(itemId, activeBackendProfile);
  ...
  async function handleClearQueue(mode: "finished" | "all") {
    try {
      const items = await clearLitegraphQueue(mode, activeBackendProfile);
              ...
              {litegraphWorkflows.length === 0 ? (
                ...
                litegraphWorkflows.map((item) => (
                  <button
                    ...
                    onClick={async () => {
                      try {
                        const payload = await loadLitegraphWorkflow(item.name, activeBackendProfile);
