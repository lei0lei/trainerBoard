...
function createServerProvider(backendProfile?: BackendConnectionProfile | null): FileSystemProvider {
  return {
    source: "server",
    capabilities: {
      canRead: true,
    ...
    readFile: async (node) => fetchServerFile(node.path, backendProfile),
    readDirectory: async (path) => (await fetchServerWorkspace(path, 1, backendProfile))?.children ?? [],
    readWorkspace: async (path, maxDepth = 1) => fetchServerWorkspace(path, maxDepth, backendProfile),
    saveFile: async (path, content) => {
      await saveServerFile(path, content, backendProfile);
    },
    createFile: async (parentPath, name) => createServerFile(joinPath(parentPath, name), "", backendProfile),
    createDirectory: async (parentPath, name) => createServerDirectory(joinPath(parentPath, name), backendProfile),
    renameNode: async (path, nextName) => renameServerNode(path, nextName, backendProfile),
    deleteNode: async (path) => deleteServerNode(path, backendProfile),
    canSaveFile: () => true,
  };
}
...
export function getFileSystemProvider(workspace: WorkspaceRoot, backendProfile?: BackendConnectionProfile | null): FileSystemProvider | null {
  if (!workspace?.source) return null;
  return workspace.source === "server" ? createServerProvider(backendProfile) : localProvider;
}
