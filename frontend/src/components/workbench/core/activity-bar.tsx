...
import { activityContributions } from "./activity-registry";
        ...
        {activityContributions.map((item) => (
          <button
            ...
            className={cn(
              ...
              activeSidebar === item.manifest.key && showPrimarySidebar && "text-white"
            )}
          >
            {activeSidebar === item.manifest.key && showPrimarySidebar && <span className="absolute left-0 top-1/2 h-6 w-0.5 -translate-y-1/2 rounded bg-white" />}
