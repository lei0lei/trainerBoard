import { FilePlus2, FolderOpen, FolderPlus, PanelLeftOpen, RefreshCw } from "lucide-react";
import { Button } from "@/components/ui/button";
import { placeholderByActivity } from "../core/config";
import { ExplorerTree } from "./explorer-tree";
import type { FileNode, SidebarKey, WorkspaceRoot } from "../core/types";
...
export function PrimarySidebar({
  ...
  if (activity !== "explorer") {
    const placeholder = placeholderByActivity[activity as keyof typeof placeholderByActivity] ?? {
      title: "View is empty",
      description: "This activity can provide its own primary sidebar later.",
    };
    return (
      <aside className="flex h-full min-h-0 flex-col bg-[#252526]">
        ...
        <div className="flex flex-1 flex-col justify-center gap-3 px-4 text-sm text-[#9d9d9d]">
          <PanelLeftOpen className="h-8 w-8 text-[#6f6f6f]" />
